    ...
    public function cek_status_pembayaran_pg()
{
    ...
    // Ambil daftar pembayaran PG yang belum sukses (status_code ≠ 200)
    ...
    foreach ($pembayaranPending as $ks) {
        ...
        // --- Perbaikan explode di sini ---
        // Bagi menjadi maksimal 2 bagian
        $parts = explode('-', $orderId, 2);

        // Jika ada dua bagian, gabungkan "prefix-suffix", 
        // kalau tidak, pakai seluruh string saja
        if (count($parts) === 2) {
            $faktur = $parts[0] . '-' . $parts[1];
        } else {
            $faktur = $parts[0];
        }
        // -----------------------------------
    ...
    for ($i = 0; $i < count($ids); $i++) {
        ...
        if (isset($outputjson['status_code']) && $outputjson['status_code'] != '404') {
            // ... (logika existing Anda: expire/cancel/deny atau update status_code 200)
            ...
            // status_code 404: reset jika masih tercatat
            DB::update(
                'UPDATE pembayaran
                 SET status_code     = NULL,
                     transaction_time= NULL,
